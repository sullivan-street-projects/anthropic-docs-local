...
# Claude Code Tips from the Team — Boris Cherny (Jan 31, 2026)

**Author:** Boris Cherny (@bcherny), Creator of Claude Code
**Source:** [Thread on X](https://x.com/bcherny/status/2017742741636321619) | [ThreadReaderApp](https://threadreaderapp.com/thread/2017742741636321619)
**Context:** Tips sourced directly from the Claude Code team. "The way the team uses Claude is different than how I use it. Remember: there is no one right way to use Claude Code -- everyone's setup is different."
...
- Boris personally uses multiple git checkouts, but most of the Claude Code team prefers worktrees — it's the reason @amorriscode built native support for them into the Claude Desktop app
...
See: [Parallel sessions with git worktrees](https://code.claude.com/docs/en/common-workflows#run-parallel-claude-code-sessions-with-git-worktrees)
...
Learn more: [Extend Claude with Skills](https://code.claude.com/docs/en/skills#extend-claude-with-skills)
...
More tips: [Terminal config](https://code.claude.com/docs/en/terminal-config)
...
Ask Claude Code to use the `bq` CLI to pull and analyze metrics on the fly. The team has a BigQuery skill checked into the codebase, and everyone uses it for analytics queries directly in Claude Code.
