...
- **WebFetch silently truncates long pages — detect it, don't ship it blind:** WebFetch summarizes/extracts and will cut off a long multi-section document mid-report without erroring (2026-09-13: the September threat-intelligence report lost its Surveillance Operations and later harm-category sections). A partial mirror looks complete unless you check. Two defenses now in place: (1) when extracting a known-long report, add a visible truncation note in the file body; (2) `scripts/validate.js` Layer 4 scans every `web-extracted` body for truncation markers ("truncated in source material", "Content continues but was truncated", etc.) and warns to re-fetch. If you see that warning, the fix is targeted multi-fetch of the missing sections, not a re-run of the same single WebFetch.
- **Verify a stalled agent from disk, never from its last message (empirical, 2026-09-13):** Agent A (aggregation pages) again hit the 600s watchdog; its final report was a truncated fragment ("Now the authentication headers table and the surrounding text:"). Recovery: `git status` its target paths, check frontmatter + last line of each written file for integrity, and independently re-verify the un-written ones (both were legitimately unchanged, so no false timestamp bump). This is the same ground-truth-over-self-report defense the new `research/alignment-assessment-cybersecurity-incidents.md` describes as the antidote to "biased reasoning."
