...
| `dontAsk`           | Denies anything not in `allowedTools`; connector tools set to `ask` and tools requiring user interaction are denied even if listed | Locked-down headless agents               |
| `bypassPermissions` | Runs every tool without prompting, unless an explicit `ask` rule matches, connector tools set to `ask`, or tools requiring user interaction. TypeScript SDK also requires `allowDangerouslySkipPermissions: true` | Sandboxed CI, fully trusted environments  |
